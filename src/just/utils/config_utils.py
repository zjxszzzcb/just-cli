...
from just.models.system_info import SystemConfig
...
def ensure_extensions_dir() -> Path:
...
def get_builtin_extensions_dir() -> Path:
    """Get built-in extensions directory path"""
    # Get parent directory of current file, then locate to src/just/extensions
    current_dir = Path(__file__).parent
    return current_dir.parent / "extensions"


def load_system_config() -> Optional[SystemConfig]:
    """Load system configuration"""
    config_file = get_system_info_file()
